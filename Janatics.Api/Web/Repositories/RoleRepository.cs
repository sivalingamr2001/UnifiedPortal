...
public class RoleRepository
{
    ...
    public async Task<IEnumerable<RoleModel>> ListAsync()
    {
        ...
        return await conn.QueryAsync<RoleModel>("JAN_PKG_JAN_ROLE_MASTER.P_LIST_ROLES", p, commandType: CommandType.StoredProcedure);
    }
    ...
    public async Task<RoleModel?> GetAsync(int roleId)
    {
        ...
        p.Add("P_CURSOR", dbType: OracleMappingType.RefCursor, direction: ParameterDirection.Output);
        var rows = await conn.QueryAsync<RoleModel>("JAN_PKG_JAN_ROLE_MASTER.P_GET_ROLE", p, commandType: CommandType.StoredProcedure);
    ...
    public async Task<ProcedureResult> CreateAsync(RoleModel role, string createdBy)
    {
        using var conn = _factory.CreateConnection();
        var p = new OracleDynamicParameters();
        p.Add("P_ROLE_CODE", null, OracleMappingType.Varchar2, ParameterDirection.Input); // always auto-generated - client-supplied codes are ignored by design
        ...
        await conn.ExecuteAsync("JAN_PKG_JAN_ROLE_MASTER.P_INSERT_ROLE", p, commandType: CommandType.StoredProcedure);
    ...
    public async Task<ProcedureResult> UpdateAsync(int roleId, RoleModel role, string modifiedBy)
    {
        ...
        p.Add("P_RESULT_CODE", dbType: OracleMappingType.Int32, direction: ParameterDirection.Output);
        p.Add("P_RESULT_MSG", dbType: OracleMappingType.Varchar2, direction: ParameterDirection.Output, size: 400);
        await conn.ExecuteAsync("JAN_PKG_JAN_ROLE_MASTER.P_UPDATE_ROLE", p, commandType: CommandType.StoredProcedure);
    ...
    public async Task<ProcedureResult> DeleteAsync(int roleId)
    {
        ...
        p.Add("P_RESULT_CODE", dbType: OracleMappingType.Int32, direction: ParameterDirection.Output);
        p.Add("P_RESULT_MSG", dbType: OracleMappingType.Varchar2, direction: ParameterDirection.Output, size: 400);
        await conn.ExecuteAsync("JAN_PKG_JAN_ROLE_MASTER.P_DELETE_ROLE", p, commandType: CommandType.StoredProcedure);
        return new ProcedureResult { Success = p.Get<int>("P_RESULT_CODE") == 1, Message = p.Get<string>("P_RESULT_MSG") };
    }
}
